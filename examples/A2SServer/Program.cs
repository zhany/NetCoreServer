...
namespace A2SServer
{
    internal class A2SServer : UdpServer
    {
        ...
        private MultiverseSdk sdk;
        public A2SServer(IPAddress address, int port) : base(address, port)
        {
            sdk = new MultiverseSdk();
        }

        protected override async void OnStarted()
        {
            bool ok = await sdk.ConnectAsync();
            await sdk.ReadyAsync();

            var envs = await sdk.GetLabelEnvs();
            foreach (var kv in envs)
            {
                Console.WriteLine($"Allocation Env: key {kv.Key}, value {kv.Value}");
            }
            ...
            await Task.Run(async () =>
            {
                while (true)
                {
                    ...
                    {
                        Console.WriteLine("health - ping ok");
                    }
                    else
                    {
                        Console.WriteLine("health - ping failed");
                    }
